...
func TestFractureMineralUC_Fracture(t *testing.T) {
	type fields struct {
	...
	tests := []struct {
			...
			fields: fields{
				GetOP: func() repository.MineralGetByIDIFace {
					...
					getMock.On("Get", mock.AnythingOfType("string")).Return(
						&domain.Mineral{State: domain.Fractured}, nil)
					return getMock
				}(),
		...
		{
			name: "cannot convert from liquid to fractures",
			fields: fields{
				GetOP: func() repository.MineralGetByIDIFace {
					getMock := &mocks.MineralGetByIDIFace{}
					getMock.On("Get", mock.AnythingOfType("string")).Return(
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := m.Fracture(tt.args.mineralID); (err != nil) != tt.wantErr {
